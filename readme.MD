...
# The scripts are all stand-alone and do not depend upon one another.
...
- The script requires your mcrcon password to be stored as RCON_PASS in /etc/minecraft.env
...
- The script searches the log files and determines which players have joined the game at least minimum_plays number of times and when they last joined. The idea here is a new player may be more likely to grief and therefore won't trigger a backup until they've played a certain number of times.  
...
## Bash script that extracts the names of all players from the logs along with the number of times they've joined, total play time, first and last time seen.
